...
public class AbyssLink extends JavaPlugin {
    ...
    protected void setup() {
        super.setup();
        System.out.println("[AbyssLink Discord] Plugin loading...");
        ...
        localeManager = new LocaleManager(dataFolder);
        ...
        discordBot.start().thenAccept(success -> {
            if (success) {
                System.out.println("[AbyssLink Discord] Successfully connected to Discord!");
                System.out.println("[AbyssLink Discord] Two-way chat bridge is active");
            } else {
                System.out.println("[AbyssLink Discord] Failed to connect to Discord");
                System.out.println("[AbyssLink Discord] Please check your configuration");
            }
        });

        System.out.println("[AbyssLink Discord] Registering event listeners...");
        ...
        System.out.println("[AbyssLink Discord] Event listeners and commands registered!");

        System.out.println("[AbyssLink Discord] Plugin enabled!");
    }

    private void onPlayerChat(PlayerChatEvent event) {
        System.out.println("[AbyssLink Discord] Chat event received!");
        System.out.println("[AbyssLink Discord] Cancelled: " + event.isCancelled());
        
        PlayerRef sender = event.getSender();
        String message = event.getContent();
        
        System.out.println("[AbyssLink Discord] Processing chat: " + sender.getUsername() + ": " + message);
    ...
    private void onPlayerJoin(com.hypixel.hytale.server.core.event.events.player.PlayerConnectEvent event) {
        ...
        System.out.println("[AbyssLink Discord] Player joined: " + playerRef.getUsername() + " (Total playtime: " + data.getFormattedPlayTime() + ")");
    ...
    private void onPlayerLeave(com.hypixel.hytale.server.core.event.events.player.PlayerDisconnectEvent event) {
        ...
        if (data != null) {
            data.endSession();
            System.out.println("[AbyssLink Discord] Player left: " + playerRef.getUsername() + " (Session time: " + (System.currentTimeMillis() - data.getLastLoginTime()) / 1000 + "s)");
        }
    ...
    private void onServerBoot(BootEvent event) {
        System.out.println("[AbyssLink Discord] Server boot event received");
    ...
    public void loadConfig() {
        ...
        if (!configFile.exists()) {
            ...
            System.out.println("[AbyssLink Discord] Created default config at: " + configFile.getAbsolutePath());
            System.out.println("[AbyssLink Discord] Please configure your bot token and channel IDs!");
        } else {
            try (FileReader reader = new FileReader(configFile)) {
                ...
                if (updated) {
                    ...
                    System.out.println("[AbyssLink Discord] Updated config with missing default settings");
                }

                config = gson.fromJson(existingJson, DiscordConfig.class);
                System.out.println("[AbyssLink Discord] Config loaded successfully");
            } catch (Exception e) {
                System.out.println("[AbyssLink Discord] Error loading config: " + e.getMessage());
                config = new DiscordConfig();
                saveConfig(configFile);
            }
        }
    ...
    public void saveConfig(File configFile) {
        try (FileWriter writer = new FileWriter(configFile)) {
            ...
            System.out.println("[AbyssLink Discord] Error saving config: " + e.getMessage());
        }
    }
